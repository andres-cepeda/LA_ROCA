...
class EmpleadoController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $usuario=DB::select('select email from usuario WHERE idUsuario = ?', [$empleado->idUsuario ]);
        $empleado->idUsuario =$usuario[0]->email;

        $eps=DB::select('select NombreEPS  from eps WHERE idEPS  = ?', [$empleado->idEPS  ]);
        $empleado->idEPS  =$eps[0]->NombreEPS;
